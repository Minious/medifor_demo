...
async function getImagesSize(imagesUrl){
	return new Promise((resolve, reject) => {
		let imagesLoadingPromises = imagesUrl.map(imageUrl => {
			let imageTag = document.createElement("img");
			let url = imageUrl;

			let imageLoadingPromise = new Promise((resolve, reject) => {
				imageTag.addEventListener("load", function(){
					resolve({imageUrl, imageTag});
				});
			});
...
async function createGraphPage(data) {
	...
	let graphData = buildGraphJson(nodes, edges);
	...
	let listImages = graphData.nodes.map(node => node.url);
	console.log(listImages)
	let imagesSizes = await getImagesSize(listImages);
	console.log(imagesSizes)

	graphData.nodes.forEach(node => {
		console.log(node.url)
		let nodeSize = clampImageSize(
			imagesSizes[node.url].width,
			imagesSizes[node.url].height,
			80,
			80
		);
...
async function createGraphPage2(data) {
	cleanPage();
